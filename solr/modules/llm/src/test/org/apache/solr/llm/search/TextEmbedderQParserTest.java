...
public class TextEmbedderQParserTest extends TestLlmBase {
  ...
  public static void init() throws Exception {
    setupTest("collection1", true, false);
  ...
  public void notExistentModel_shouldThrowException() throws Exception {
    ...
    queryParams.add("q", solrQuery);
    queryParams.add("fl", "id");
    QueryResponse response = solr.query(queryParams);
    
    assertJsonMatches(solrQuery, response.jsonStr(), json(        "/error/msg=='The model requested \\'not-exist\\' can\\'t be found in the store: /schema/embedding-model-store'"
            ), JSONTestUtil.DEFAULT_DELTA);
    assertJsonMatches(solrQuery, response.jsonStr(), json("/error/code==400"), JSONTestUtil.DEFAULT_DELTA);
  }
