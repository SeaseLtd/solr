...
The goal of sentence similarity is to encode text into a vector in such a way that similar sentences are encoded into vectors close to each other in a vector space, using a vector distance metric.
...
Apache Solr uses https://github.com/langchain4j/langchain4j[LangChain4j] to connect to such APIs.  Solr works with some but not all models that LangChain4j supports.

Currently, the only supported way to interact with Large Language Models is via embedding text.

[NOTE]
====
  Apache Solr is *in no way* affiliated to any of these corporations or services, nor is it endorsing them.
====
